...
def handle_connect_file(client,address,dict_data):
    ...
    if file_mode == 'upload_file':
        ...
        with open('./file_data/%s'%(file_name + '.tmp'), "rb") as f:
            for chunk in iter(lambda: f.read(4096), b""):
                hash_md5.update(chunk)
        if file_hash == hash_md5.hexdigest():
            sys_show_on('%s文件接收完成,文件MD5检验通过'%(file_name))
            ...
            sys_show_on('%s文件接收过程中出错,MD5校验不一致')          
    elif file_mode == 'download_file':
        file_name = dict_data['file_name']
        hash_md5 = hashlib.md5()
        if os.path.isfile('./file_data/%s'%(file_name)):
            client.send('file exist'.encode())
            ...
            with open('./file_data/%s'%(file_name), "rb") as f:
                for chunk in iter(lambda: f.read(4096), b""):
                    hash_md5.update(chunk)
                    file_hash = hash_md5.hexdigest().encode()
...
if __name__ == '__main__':
    ...
    p = multiprocessing.Process(target=main_zmq_mode,args=(zmq_xsub_port,zmq_xpub_port))
    p.daemon = True
    p.start()
